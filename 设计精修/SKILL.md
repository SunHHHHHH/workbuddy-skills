---
name: impeccable
description: Design refinement and polish for EXISTING frontend interfaces. Use AFTER the initial design/component already exists — NOT for building new UIs from scratch (use UI_UX设计 or 前端UI工程 for that). Best for: UX critique (hierarchy, clarity, cognitive load), design audit (a11y, responsiveness, anti-pattern detection like Inter font/purple gradients/cards-in-cards/gray-on-color), visual polish (typography, spacing, color harmony, motion finesse), and design system alignment. Covers bloand→bolder, loud→quieter, complex→simpler transformations. 优先级: 已有界面打磨→本技能; 新建界面→UI_UX设计; 新建组件→前端UI工程; 代码逻辑审查→代码审查; 安全漏洞→安全加固. Not for backend-only tasks.
version: 3.7.1
user-invocable: true
