...
If `prd-structure.yaml` doesn't exist, prompt the user to run `/prd-analyze` or `/ingest-docs` first.
...
1a. **Check for existing gap analysis**:
   - Search for `{plans_dir}/*-gap-analysis.yaml`. If found, load it into memory as `existing_gap_analysis`.
   - This enables merge mode in Step 5 (updating existing gaps with fresh coverage data instead of overwriting them).
...
### Step 5: Output (with Merge Logic)

**If `existing_gap_analysis` is loaded (merge mode):**

1. For each gap in `existing_gap_analysis.gaps`:
   - Run Steps 2-3 (scan codebase) to find fresh coverage evidence
   - Add/update these fields per gap:
     ```yaml
     coverage_evidence:
     ...
     ```
   - **Preserve** existing fields: `source_id`, `description`, `priority`, `missing_parts`, `estimated_effort`, `depends_on_gaps`
   - **Update** `gap_type` from the fresh scan: if `coverage_evidence.status` is `missing`, set `gap_type: missing`; if `partial`, set `gap_type: partial`; if `implemented`, remove the gap or mark it `gap_type: resolved`. This prevents stale human classifications from contradicting fresh scan evidence.

2. For any user stories in `prd_structure` NOT covered by existing gaps:
   - **Matching rule**: a user story is "covered" when any existing gap has `story_id == user_story.id`. If a gap has no `story_id` field, fall back to matching `gap.module == user_story.module` AND the gap's `description` mentions the story's `action` keyword.
   - For uncovered stories: run full gap detection (Steps 2-3) and **APPEND** new gaps to the list

3. Add a `scan_summary` section to the output YAML:
   ```yaml
   scan_summary:
...
4. Set `source_type: "ingested+scanned"` in the output YAML

5. Write to `{plans_dir}/{date}-gap-analysis.yaml`

6. Print merge-mode summary to terminal:
...
2. Set `source_type: "scanned"` in the output YAML
3. Print full-scan summary to terminal:

```
Gap Analysis Complete (full scan)
=================================
Coverage: 62% (28/45 stories implemented)
...
📋 Next:
  /task-gen    — Generate task list from gap analysis
  /plan-schedule    — Create execution plan with batches (if tasks.yaml exists)
─────────────────────────────────────────────────────
