...
### Step 3.N: Generate Test Requirements (0.7.0+)

For each non-Red task generated in the previous step, derive `test_requirements`
from the linked acceptance criteria.

**Input sources:**
- `gap_ref` → gap-analysis.yaml → `gaps[].acceptance_criteria` or `gaps[].expected_behavior`
- `story_refs` → prd-structure.yaml → `user_stories[].acceptance_criteria`
- `must_call_unchanged` → contract symbols (for integration layer)
...
📋 Next:
  /plan-schedule    — create batches and milestones
  /next-task    — start working immediately
─────────────────────────────────────────────────────
